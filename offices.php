<?php include('./header.php'); ?>

<main>

    <section>
        <section>
            <header class="header-default">
                <h2 class="title-default">Conheça nossos escritórios</h2>
            </header>
        </section>
    </section>

    <section>
        slides offices
    </section>

    <section class="cta_find_office">
        <div class="container d-flex">
            <div class="f-50">
                <h2 class="title-default">Encontre uma MRD mais próximo de você</h2>
                <p class="desc-default">Preencha os dados ao lado para encontrar o escritório mais próximo da sua localidade.</p>
            </div>

            <div class="f-50">
                <form action="" class="form_office d-flex">
                    <div class="form_group f-100">
                        <input type="text" name="" id="" placeholder="Cidade">
                    </div>
                    <div class="form_group f-50">
                        <input type="text" name="" id="" placeholder="CEP">
                    </div>
                    <div class="form_group f-50">
                        <input type="text" name="" id="" placeholder="Estado">
                    </div>
                    <div class="form_group w-100 form_group_button">
                        <button class="btn-blue" type="submit">Buscar</button>
                    </div>
                </form>
            </div>
        </div>
    </section>

    <section class="office_uni">

        <article class="card_office_uni d-flex">
            <div class="f-50 office_uni_img">
                <img src="assets/img/office.jpg" alt="">
            </div>

            <div class="f-50 office_uni_info">

                <div class="office_content">

                    <h3 class="office_tag">SEDE</h3>

                    <header class="office_header_info">
                        <i class="bi bi-geo-alt-fill title-default"></i> 
                        <h2 class="title-default">Escritório de Advocacia em Belo Horizonte/MG</h2>
                    </header>

                    <div class="office_contact">
                        <div class="office_contact_row">
                            <span>Endereço:</span>
                            <p class="desc-default">Rua do Ouro, 558, Serra - Belo Horizonte/MG</p>
                        </div>

                        <div class="office_contact_row">
                            <span>CEP:</span>
                            <p class="desc-default">30.220-000</p>
                        </div>

                        <div class="office_contact_row">
                            <span>Tel:</span>
                            <p class="desc-default">(31) 0000 00 00</p>
                        </div>
                    </div>

                    <img src="assets/img/mapa_BH 1.jpg" alt="">

                </div>

            </div>
        </article>
        

        <article class="card_office_uni d-flex">
            <div class="f-50 office_uni_img">
                <img src="assets/img/office2.jpg" alt="">
            </div>

            <div class="f-50 office_uni_info">

                <div class="office_content">

                    <header class="office_header_info">
                        <i class="bi bi-geo-alt-fill title-default"></i> 
                        <h2 class="title-default">Escritório de Advocacia em Belo Horizonte/MG</h2>
                    </header>

                    <div class="office_contact">
                        <div class="office_contact_row">
                            <span>Endereço:</span>
                            <p class="desc-default">Rua do Ouro, 558, Serra - Belo Horizonte/MG</p>
                        </div>

                        <div class="office_contact_row">
                            <span>CEP:</span>
                            <p class="desc-default">30.220-000</p>
                        </div>

                        <div class="office_contact_row">
                            <span>Tel:</span>
                            <p class="desc-default">(31) 0000 00 00</p>
                        </div>
                    </div>

                    <img src="assets/img/mapa_BH 1.jpg" alt="">

                </div>

            </div>
        </article>


        <article class="card_office_uni d-flex">
            <div class="f-50 office_uni_img">
                <img src="assets/img/office3.jpg" alt="">
            </div>

            <div class="f-50 office_uni_info">

                <div class="office_content">

                    <header class="office_header_info">
                        <i class="bi bi-geo-alt-fill title-default"></i> 
                        <h2 class="title-default">Escritório de Advocacia em Belo Horizonte/MG</h2>
                    </header>

                    <div class="office_contact">
                        <div class="office_contact_row">
                            <span>Endereço:</span>
                            <p class="desc-default">Rua do Ouro, 558, Serra - Belo Horizonte/MG</p>
                        </div>

                        <div class="office_contact_row">
                            <span>CEP:</span>
                            <p class="desc-default">30.220-000</p>
                        </div>

                        <div class="office_contact_row">
                            <span>Tel:</span>
                            <p class="desc-default">(31) 0000 00 00</p>
                        </div>
                    </div>

                    <img src="assets/img/mapa_BH 1.jpg" alt="">

                </div>

            </div>
        </article>

        <div class="load_more_office">
            <img src="assets/img/loading.png" alt="">
        </div>

    </section>

</main>

<?php include('./footer.php'); ?>